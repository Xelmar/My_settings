...
prompt_command_execution_time() {
  ...
  if (( _P9K_COMMAND_DURATION >= POWERLEVEL9K_COMMAND_EXECUTION_TIME_THRESHOLD )); then
    "$1_prompt_segment" "$0" "$2" "green" "226" "${humanReadableDuration}" 'EXECUTION_TIME_ICON'  #___________________________
  fi
}
...
prompt_dir() {
  ...
  "$1_prompt_segment" "$0_${current_state}" "$2" "cyan" "$DEFAULT_COLOR" "${current_path}" "${dir_states[$current_state]}" #__________________________
}
...
prompt_status() {
  if [[ "$RETVAL" -ne 0 ]]; then
    if [[ "$POWERLEVEL9K_STATUS_VERBOSE" == true ]]; then
      "$1_prompt_segment" "$0_ERROR" "$2" "088" "015" "$RETVAL" 'CARRIAGE_RETURN_ICON' #______________________________________________
    else
      "$1_prompt_segment" "$0_ERROR" "$2" "088" "015" "" 'FAIL_ICON' #___________________________________________________
    fi
  elif [[ "$POWERLEVEL9K_STATUS_VERBOSE" == true || "$POWERLEVEL9K_STATUS_OK_IN_NON_VERBOSE" == true ]]; then
    "$1_prompt_segment" "$0_OK" "$2" "015" "cyan" "" 'OK_ICON' #___________________________________$DEFAULT_COLOR
  fi
}
...
prompt_swap() {
  ...
  "$1_prompt_segment" "$0" "$2" "015" "$DEFAULT_COLOR" "$(printSizeHumanReadable "$swap_used" $base)" 'SWAP_ICON' #_________________________________________________
}
...
prompt_time() {
  ...
  "$1_prompt_segment" "$0" "$2" "238" "$DEFAULT_COLOR" "$time_format"  #_______________$DEFAULT_COLOR_INVERTED
}
